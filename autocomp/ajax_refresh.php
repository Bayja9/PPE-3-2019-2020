...
$sql = "SELECT * FROM chevaux WHERE nom_chevaux LIKE (:keyword) ORDER BY nom_chevaux ASC LIMIT 0, 10";  // création de la requete avec sélection des résultats sur la lettre
...
foreach ($list as $res) {
	...
	// sélection
    echo '<li onclick="set_item(\''.str_replace("'", "\'", $res['nom_chevaux'].' '.$res['couleur_chevaux']).'\')">'.$Listeeleve.'</li>';
}
?>
